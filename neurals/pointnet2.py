...
class PointNetConv(nn.Module):
    ...
    def forward(self, nodes, batchsize):
        shape = nodes.mailbox['agg_feat'].shape
        h = nodes.mailbox['agg_feat'].view(batchsize, -1, shape[1], shape[2]).permute(0, 3, 2, 1)
...
class SAModule(nn.Module):
    ...
    """
    def __init__(self, npoints, radius, mlp_sizes, n_neighbor=64,
                 group_all=False):
        ...
        self.conv = PointNetConv(mlp_sizes)
    ...
    def forward(self, pos, feat):
        ...
        partial_conv = partial(self.conv, batchsize=batch_size)
        g.update_all(self.message, partial_conv)
        ...
        feat_res = g.ndata['new_feat'][mask].view(batch_size, feat_dim, -1)
