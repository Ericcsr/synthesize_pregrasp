...
def main():
    ...
    score_function = LargeScoreFunction(num_fingers=2).cuda()
    loss_fn = nn.MSELoss()
    optimizer = torch.optim.Adam(score_function.parameters(), lr=1e-3)

    # TODO: Make kin_feasibility and dyn_feasibility working
    full_dataset = neurals.dataset.ScoreDataset(score_file=opt.dataset_file)
    ...
    train_loader = torch.utils.data.DataLoader(
        training_dataset, batch_size=16)
    ...
    for epoch in range(2100):
        ...
        if epoch % 20 == 0:
            print('saving the latest model (epoch %d, total_steps %d)' %
                    (epoch, total_steps))
            torch.save(score_function.state_dict(), f"{current_dir}/only_score_model_{opt.exp_name}_{epoch}.pth")
            iter_data_time = time.time()
