...
def main():
    ...
    model = dgn.DexGraspNetRegressor(opt, 
                                     pred_base=False,
                                     pred_fingers=[0,2], 
                                     extra_cond_fingers=[1], 
                                     gpu_id=0)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(train_loader):
            ...
            if total_steps % opt.print_freq == 0:
                loss_types = []
                if opt.arch == "regressor":
                    # For vae, reconstruction loss is just the L1 loss
                    loss = [
                        model.loss_train, model.fingertip_pos_loss_train
                    ]
                    loss_types = [
                        "total_loss", "fingertip_pos_loss"
                    ]
                else:
