...
if __name__ == "__main__":
    ...
    feasible_output_path = '/'.join([os.path.dirname(os.path.abspath(__file__)),"..",
                            dgc.dyn_feasible_points_path])
    infeasible_output_path = '/'.join([os.path.dirname(os.path.abspath(__file__)),"..",
                            dgc.dyn_infeasible_points_path])

    input_path = '/'.join([os.path.dirname(os.path.abspath(__file__)),"..",
                            dgc.point_cloud_path])
    file = args.drake_obj_name+"_textured_"+str(dgc.num_points_in_point_cloud)+".ply"
    output_name = file.split('.')[0]
    # Point clouds are stored with mesh_sdf_xyz_rpy considered
    print('Computing '+file)
    drake_obj_name = "_".join(output_name.split("_")[:-2])
    print('drake_object_name',drake_obj_name)
    # Load the poses
    rest_pose_path = '/'.join([os.path.dirname(os.path.abspath(__file__)),'..',
            dgc.rest_pose_path, drake_obj_name+'.npy'])
    feasible_output_dir = os.path.join(feasible_output_path, output_name)
    infeasible_output_dir = os.path.join(infeasible_output_path, output_name)
    ...
    rest_poses = np.load(rest_pose_path)
    np.save(os.path.join(feasible_output_dir, "poses"), rest_poses)
    np.save(os.path.join(infeasible_output_dir, "poses"), rest_poses)
    # For each rest pose, we iterate through all triples to find those that are dynamically feasible
    for idx, rest_pose in enumerate(rest_poses):
        ...
        pc = o3d.io.read_point_cloud(os.path.join(
            os.path.dirname(os.path.abspath(__file__)),"..",
            dgc.point_cloud_path,file))
        print('Storing to '+ feasible_output_path_name, "and", infeasible_output_path_name)
        # Transform the point cloud
        T_WB = np.eye(4)
        T_WB[:3,:3] = scipy_rot.from_quat(rest_pose[:4][[1,2,3,0]]).as_matrix()
        T_WB[:3,-1] = rest_pose[4:]
        dyn_feasible, dyn_infeasible = find_dynamically_feasible_contacts(pc, num_procs=args.num_process,n_contacts=args.num_contacts)
