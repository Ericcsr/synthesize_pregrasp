...
def fit_interp_ik(targets,
                  ...
                  obj_orns,
                  arm_drake,
                  joint_keyframe_start,
                  joint_keyframe_end):
    joint_states = []
    success_flags = []
    desired_positions = []
    pcs =  []
    joint_interp = np.linspace(joint_keyframe_start,joint_keyframe_end,6)
    arm_drake.regularize = True
    for i in range(len(targets)):
        pcs.append(arm_drake.setObjectPose(obj_poses[i], obj_orns[i]))
        _, joint_state, _, desired_position, success = arm_drake.regressFingerTipPosWithRandomSearch(
            targets[i],
            weights[i],
            has_normal=True,
            prev_q=helper.convert_joints_for_drake(joint_interp[i]),
            n_process=64)
        joint_states.append(joint_state)
        success_flags.append(success)
        desired_positions.append(desired_position)
    ...
    for i in range(len(targets)):
        ...
        if i==0:
            q_sol, joint_state, _, desired_position, success = arm_drake.regressFingerTipPosWithRandomSearch(
                ...
                weights[i],
                has_normal=True,
                prev_q=helper.convert_joints_for_drake(joint_keyframe_start),
                n_process=64)
        else:
            q_sol, joint_state, _, desired_position, success = arm_drake.regressFingerTipPosWithRandomSearch(
                targets[i],
...
if __name__ == "__main__":
    ...
    key_joint_states = np.load("data/redo_2_50_0.8_keyframes.npy")
    ...
    # np.save(f"data/{args.exp_name}_keyframes.npy", joint_states)
    ...
    # np.save(f"data/{args.exp_name}_segment1.npy", joint_states)
    ...
    #                         arm_drake,
    #                         renderer=renderer) # Good to go
    # np.save(f"data/{args.exp_name}_segment2.npy", joint_states)
    ...
    #                         arm_drake,
    #                         renderer=renderer) # Good to go
    # np.save(f"data/{args.exp_name}_segment3.npy", joint_states)
    
    # Interpolate and animate all the frames..

    segment_1 = np.load("data/redo_2_50_0.8_segment1.npy")
    segment_2 = np.load("data/redo_2_50_0.8_segment2.npy")
    segment_3 = np.load("data/redo_2_50_0.8_segment3.npy")
    ...
    np.save("data/redo_2_50_0.8_full_states.npy", full_joint_states)
